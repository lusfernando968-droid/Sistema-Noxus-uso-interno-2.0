...
create policy if not exists aula_modelos_select on public.aula_modelos for select using (user_id = auth.uid());
create policy if not exists aula_modelos_insert on public.aula_modelos for insert with check (user_id = auth.uid());
create policy if not exists aula_modelos_update on public.aula_modelos for update using (user_id = auth.uid()) with check (user_id = auth.uid());

create policy if not exists aulas_select on public.aulas for select using (user_id = auth.uid());
create policy if not exists aulas_insert on public.aulas for insert with check (user_id = auth.uid());
create policy if not exists aulas_update on public.aulas for update using (user_id = auth.uid()) with check (user_id = auth.uid());

create policy if not exists aula_versions_select on public.aula_versions for select using (user_id = auth.uid());
create policy if not exists aula_versions_insert on public.aula_versions for insert with check (user_id = auth.uid());
